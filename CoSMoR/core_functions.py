...
# -----------------------------------------------
def create_alloys(comp_A, comp_B, dc = 0.01, cAmin = 0.0, cAmax = 1.0, replace_el = None):
    ...
    for (i, comp) in zip(range(len(comp_array)), comp_array):
        alloy_name = ""
        xA = comp[0]  # total fraction of component A being added

        for (el, stoich) in zip(A_comp_dict["el_list"], A_comp_dict["el_at_frac_list"]):
            el_at_frac = round(xA * stoich, 4)
        ...
        for (el, stoich) in zip(B_comp_dict["el_list"], B_comp_dict["el_at_frac_list"]):
            if replace_el is not None and el == replace_el:
                # xA is taken entirely from this element; others in B are unaffected
                el_at_frac = round(stoich - xA, 4)
            else:
                el_at_frac = round(comp[1] * stoich, 4)
            el_at_frac = max(el_at_frac, 0.0)  # clamp to zero, no negatives
            alloy_name += str(el) + str(el_at_frac)
            x_el_dict[el].append(el_at_frac)
...
def create_features(df_alloys):
    ...
    df_cbfv = df_alloys[["alloy_name","target"]].copy()
    df_cbfv.columns = ["formula","target"]
    x_cbfv, y_cbfv, formula_cbfv, _ = composition.generate_features(df_cbfv, elem_prop = 'oliynyk')
    feats_from_cbfv = ["avg_number_of_valence_electrons", "dev_Pauling_Electronegativity","avg_Cohesive_energy"]
    df_feats = x_cbfv.copy()[feats_from_cbfv]
    H_mied = np.array([get_miedema_enthalpy(alloy) for alloy in df_alloys["alloy_name"]])
    # create asymmetry features
    df_feats["r_asymm"] = get_asymmetry(alloy_name_list, feat_key="r_cov")
    df_feats["H_chem"] = H_mied[:,0]
    df_feats["H_el"] = H_mied[:,1]
    ...
    xmin = pd.read_csv(f"xmin.csv")
    xmax = pd.read_csv(f"xmax.csv")
...
def make_predictions(df_x_feats):
    ...
    model_list = [f for f in os.listdir(model_dir) if f.endswith('.pkl')]
    ensemble_size = len(model_list)
    
    Y_total = np.zeros(shape=(len(df_x_feats),))
    for mod in sorted(model_list):
        with open(model_dir + mod, 'rb') as f:
            model = pickle.load(f)
        
        Y_current_mod = model.predict(df_x_feats)
